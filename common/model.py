...
class SwiftAI:
    ...
    def write_songs(self, model, song_length=176, filename='songs.txt', preset_seed=None):
        ...
        :param preset_seed: if we don't want to manually input our seed, we can specify a preset seed, and we won't
        have to write our own during runtime. However, the code doesn't check if it's valid or not. Mostly used
        for testing when away from machine.
